...
async function handleClientResponse(client, message) {
    ...
    if (pedido.estado === 'iniciar' && ConfirmoVeiculos.test(message.body.toLowerCase())) {
        ...
        client.sendMessage(message.from, `Perfeito, *${pedido.nome}*! Seu agendamento foi feito para o carro *${pedido.TestDrive.carro}* no dia *${pedido.TestDrive.dia}* às *${pedido.TestDrive.hora}*.\n_Número de protocolo: ${protocolo}_`);
