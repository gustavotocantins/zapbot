...
function start(client) {
    client.onMessage((message) => {
        if (!message.isGroupMsg) {
            // Verifica se o cliente já está em atendimento
            if (!clientsInProgress[message.from]) {
                ...
                client
                    .sendText(message.from, 'Olá! Bem-vindo ao nosso atendimento. Como posso ajudar?\n\n1 - Ver Menu\n2 - Falar com Atendente\n3 - Sair')
...
function handleClientResponse(client, message) {
    ...
    switch (message.body.toLowerCase()) {
        ...
        case '2': // Opção 2 - Falar com Atendente
            if (clientState.stage === 1) {
                client
                    .sendText(message.from, 'Um atendente estará com você em breve.')
        ...
        case '3': // Opção 3 - Sair
            client
                .sendText(message.from, 'Obrigado por entrar em contato. Até logo!')
                .then(() => {
        ...
        case 'voltar': // Retorna ao menu inicial
            if (clientState.stage === 2) {
                client
                    .sendText(message.from, 'Voltando ao menu principal. Como posso ajudar?\n\n1 - Ver Menu\n2 - Falar com Atendente\n3 - Sair')
        ...
        default:
            ...
            if (clientState.stage === 2 && (message.body.toLowerCase() === 'pizza' || message.body.toLowerCase() === 'hambúrguer' || message.body.toLowerCase() === 'refrigerante')) {
                client
                    .sendText(message.from, `Pedido recebido: ${message.body}. Estamos preparando!`)
                ...
                client
                    .sendText(message.from, 'Desculpe, não entendi. Por favor, escolha uma das opções:\n\n1 - Ver Menu\n2 - Falar com Atendente\n3 - Sair')
